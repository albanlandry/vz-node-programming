...
export interface ErrorHandlingConfig {
  ...
  enableDLQ?: boolean;
  /** Fallback function */
  fallbackFn?: (error: Error, context: ExecutionContext) => Promise<Map<PortId, unknown>>;
...
export abstract class ErrorHandlingNode extends BaseNode {
   ...
   */
  private async executeWithErrorHandling(context: ExecutionContext): Promise<Map<PortId, unknown>> {
    const executeFn = () => {
      // Use circuit breaker if provided
      if (this.errorConfig.circuitBreaker) {
        return this.errorConfig.circuitBreaker.execute(() => {
    ...
    try {
      ...
      if (this.errorConfig.fallbackFn) {
        ...
        return this.errorConfig.fallbackFn(error instanceof Error ? error : new Error(String(error)), context);
      }
